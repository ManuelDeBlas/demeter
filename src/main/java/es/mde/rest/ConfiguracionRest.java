 ...
 * <ol>
 * <li>{@link #addSearchLinks(RepositoryRestConfiguration)}: enlaza cada {@code /recursos/search}
 * automaticamente con los metodos de los controladores registrados.</li>
    ...
    return new RepresentationModelProcessor<RepositorySearchesResource>() {
      ...
      public RepositorySearchesResource process(RepositorySearchesResource searchResource) {
        if (controllersRegistrados.containsKey(searchResource.getDomainType())) {
          ...
          Stream.of(metodos).filter(m -> m.isAnnotationPresent(ResponseBody.class)
              && m.isAnnotationPresent(GetMapping.class)).map(m -> {
                Link link = null;
                try {
                  String pathMetodo = String.join("", m.getAnnotation(GetMapping.class).value());
                  String pathRecurso = new URI(uriController.getScheme(),
                      uriController.getUserInfo(), uriController.getHost(), uriController.getPort(),
